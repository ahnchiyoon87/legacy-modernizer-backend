...
async def fetch_variable_nodes(node_id, variable_dict, variable_node):
    try:
        ...
        for node in variable_nodes:
            ...
            role_description = re.sub(r'합니다$|함$|됨$', '', role_start).strip()


            # * 중복된 역할 제거 후 리스트에 추가
            if var_name not in variable_dict:
                variable_dict[var_name] = [role_description] 
            elif role_description not in variable_dict[var_name]:
                variable_dict[var_name].append(role_description)
