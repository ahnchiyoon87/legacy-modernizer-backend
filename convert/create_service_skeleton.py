...
async def execute_maven_commands(pom_directory: str) -> None:
    try:
        ...
        plsql_file_path = 'data/plsql/calculate_payroll.sql'
        plsql_log_path = 'logs/plsql_logs.jsonl'
        java_log_path = 'logs/java_logs.jsonl'

        # 파일 내용 읽기
        plsql_code = read_file(plsql_file_path)
        java_files = get_all_java_files_in_directory('target')
        plsql_log = read_file(plsql_log_path)
        java_log = read_file(java_log_path)

        # 코드 분석 실행
        analyze_code(plsql_code, java_files, plsql_log, java_log, comparison_result)
...
def generate_prompt(plsql_code, java_files, plsql_log, java_log, comparison_result):
    return f"""
...
def analyze_code(plsql_code, java_files, plsql_log, java_log, comparison_result):
    try:
        # 프롬프트 생성
        prompt = generate_prompt(plsql_code, java_files, plsql_log, java_log, comparison_result)
