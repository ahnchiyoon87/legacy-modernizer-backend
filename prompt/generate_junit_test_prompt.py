...
"""
당신은 Java MyBatis JUnit 테스트 코드 생성 전문가입니다. XML 매퍼 파일과 프로시저 호출, 데이터 변경 로그를 분석하여 완벽한 테스트 코드를 생성해야 합니다.
...
- 각 테이블에 대해 다음 형식으로 import와 Repository 선언 필요:
  ...
  - XML 매퍼 파일 위치:
    src/main/resources/mapper/[테이블명PascalCase]Mapper.xml
...
4. Then 섹션 작성:
   ...
   - 데이터베이스 상태 검증을 위한 Mapper 조회 메서드 사용

5. 테스트 케이스 명명:
   - 테스트 클래스명은 핵심 시나리오 값을 포함하여 구체적으로 작성
   - "[프로시저명]_[주요입력값]_[핵심결과값]Test" 형식 사용
    * 예 핵심 결과값은 Then을 보고 판단하세요. 반드시 수치가 들어가야합니다. 
   - 클래스 명은 30 글자를 넘기지 마세요.
   - @Transactional 어노테이션을 사용하지마세요.
...
6. 데이터 타입 규칙:
   ...
   - 날짜/시간:
     ...
     - Date 키워드만 있는 경우 LocalDate 타입으로 날짜만 저장
...
{{
    ...
    "testCode": "전체 테스트 코드 (XML 매퍼 파일 포함)"
}}
"""
)
...
# 역할 : Junit 테스트 코드 생성하는 LLM 호출
#
# 매개변수 : 
#   - table_names : 테이블 이름 리스트
#   - package_name : 패키지 이름
#   - procedure_name : 프로시저 이름
#   - repository_codes : Repository 메서드 목록
#   - procedure_info : 프로시저 정보
#   - given_log : Given 로그 데이터
#   - then_log : Then 로그 데이터
#   - orm_type : 사용할 ORM 유형
#
# 반환값 : 
#   - str : 생성된 Junit 테스트 코드
def generate_test_code(table_names: list, package_name: str, procedure_name: str, repository_codes: dict, procedure_info: dict, given_log: dict, then_log: dict, orm_type: str) -> str:
    try:
        # * 프레임워크별 프롬프트 선택
        ...
        err_msg = f"테스트 코드 생성 중 오류가 발생했습니다: {str(e)}"
